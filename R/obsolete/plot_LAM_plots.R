...
output_viz_dir <- here("output", "plots", "LAM_plots")
...
plot_LAM_pro <- function(df, subject_id, lam_val, fixed_eps) {
  ...
  # Create LAM Data using the GLOBAL threshold!
  ...
  ggplot(rec_df, aes(x = Time_A, y = Time_B)) +
    ...
    labs(
      title = paste0("LAM Plot (Subject ", subject_id, ")"),
...
# 4. --- Main Loop ---
message("🎨 Generating Professional LAM Plots...")
...
for (i in seq_along(file_list)) {
  ...
  p <- plot_LAM_pro(df_points, subject_name, lam_value, global_epsilon)
  
  # Save
  plot_filename <- path(output_viz_dir, paste0(subject_name, "_LAMPlot_Pro.png"))
  ggsave(plot_filename, p, width = 6, height = 6.2, dpi = 300)
}

message("🎉 Done! Check output/plots/LAM_plots/")